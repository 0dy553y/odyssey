...
challenge_running = Challenge.create(category_id: category_exercise.id, name: 'Couch to 5k',
                                     ...
                                     duration: 30)
UserChallenge.create(user_id: user_unclesoo.id, challenge_id: challenge_running.id, started_at: 1.week.ago)
...
UserChallenge.create(user_id: user_unclesoo.id, challenge_id: challenge_drinking.id, started_at: 2.weeks.ago)
...
30.times do |i|
  ...
  UserTask.create(user_id: user_unclesoo.id, task_id: task.id, is_completed: i < 1.week.in_days)
end
...
30.times do |i|
  ...
  task = Task.create(challenge_id: challenge_drinking.id, name: "Day #{i + 1}: Bling Blong",
                     description: description, index: i)
  UserTask.create(user_id: user_unclesoo.id, task_id: task.id, is_completed: i < 2.weeks.in_days)
end
