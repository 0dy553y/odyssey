...
task update_maps_for_challenges: :environment do
  # FOR REFERENCE:
  # LAND: grass = 0, sand = 1, stone = 2, beehive = 3
  ...
  Challenge.find_by(name: 'Back to School').map.update!(land: 1, building_block: 0, background: 0)
  ...
  Challenge.find_by(name: 'Healthy Eating').map.update!(land: 2, building_block: 0, background: 0)
  Challenge.find_by(name: 'LEGO').map.update!(land: 3, building_block: 0, background: 0)
  ...
  Challenge.find_by(name: 'Poetry').map.update!(land: 2, building_block: 0, background: 0)
